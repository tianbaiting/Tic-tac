...
void calculate_Gtilde_array(double* Gtilde_array,
	...
	#pragma omp parallel
	{
		...
		for (long long p_idx=0; p_idx<Np; p_idx++){
			for (long long q_idx=0; q_idx<Nq; q_idx++){
				for (long long x_idx=0; x_idx<Nx; x_idx++){
					for (long long alpha=0; alpha<Nalpha; alpha++){
						for (long long alpha_p=0; alpha_p<Nalpha; alpha_p++){
...
void calculate_Gtilde_subarray_polar(double* Gtilde_subarray,
	...
	// [EN] For one fixed row packet (q',p') and one pair of channel labels (alpha',alpha), Gtilde is the smooth
	// geometry kernel that remains after the discrete packet bins absorb the singular Jacobi-coordinate change. The
	// later packet integral reuses this phi-x table many times. / [CN] 对于固定的行波包 (q',p') 和一对通道标签
	// (alpha',alpha)，Gtilde 是在离散波包吸收了奇异 Jacobi 坐标变换之后剩下的平滑几何核；后面的波包积分会多次复用
	// 这张 phi-x 表。
	long int fullsize = Nphi*Nx;
	long int counter = 0;
	int frac_n, frac_o=0;
	
	for (long long phi_idx=0; phi_idx<Nphi; phi_idx++){
		for (long long x_idx=0; x_idx<Nx; x_idx++){
...
// [EN] Before integrating any non-zero permutation element, we first ask whether the two incoming/outgoing packet
// cells overlap at all under the Jacobi-coordinate transformation generated by P. This cheap geometric mask is the
// first sparsity filter in the paper's finite-dimensional P-matrix construction. / [CN] 在计算任何非零置换矩阵元之前，
// 先判断两个入射/出射波包单元在 P 诱导的 Jacobi 坐标变换下是否存在几何重叠；这是论文中有限维 P 矩阵构造的第一层廉价稀疏筛选。
void calculate_WP_overlap(bool* pq_WP_overlap_array,
	...
	for (size_t qp_idx_WP=0; qp_idx_WP<Nq_WP; qp_idx_WP++){
		...
		for (size_t pp_idx_WP=0; pp_idx_WP<Np_WP; pp_idx_WP++){
			...
			/* Create phi-mesh */
			calc_gauss_points ( &phi_array[phi_base_idx],
				...
				for (size_t p_idx_WP=0; p_idx_WP<Np_WP; p_idx_WP++){
					...
					if (phi_lower<phi_upper){
						...
						for (size_t phi_idx=0; phi_idx<Nphi; phi_idx++){
							double phi = phi_array[phi_base_idx + phi_idx];
...
void calculate_permutation_elements_for_3N_channel(double** P123_val_dense_array,
	...
	// [EN] Gtilde is evaluated many times for one channel block, so Clebsch-Gordan and associated Legendre data are
	// tabulated once and then reused. This mirrors the paper's strategy of separating universal angular algebra from
	// packet-dependent quadrature. / [CN] 对单个通道块而言，Gtilde 会被反复评估，因此 Clebsch-Gordan 和缔合勒让德数据
	// 都先做一次表，再反复复用；这正对应论文里把“通用角动量代数”和“依赖波包的求积”分离开的策略。
	...
	// [EN] At this point the code has enough information to estimate the structural sparsity of P123 before filling
	// any numeric values. / [CN] 到这里为止，代码已经有足够信息在写入任何数值前预估 P123 的结构性稀疏度。
	...
	// [EN] The packet permutation integral repeatedly needs the same transformed angles costheta_1 and costheta_2.
	// Tabulating the associated Legendre data once converts the expensive inner loop into table lookups plus
	// quadrature weights. / [CN] 波包置换积分会反复用到同一批变换角 costheta_1 和 costheta_2；把相关联的勒让德数据
	// 预先制表，可以把昂贵的内层循环变成“查表 + 求积权重”。
	...
	for (size_t qp_idx_WP=0; qp_idx_WP<Nq_WP; qp_idx_WP++){
		for (size_t pp_idx_WP=0; pp_idx_WP<Np_WP; pp_idx_WP++){
			...
			for (size_t phi_idx=0; phi_idx<Nphi; phi_idx++){
				...
				for (int x_idx=0; x_idx<Nx; x_idx++){
					double x = x_array[x_idx];
					double p = sin_phi_array[phi_base_idx + phi_idx];
					double q = cos_phi_array[phi_base_idx + phi_idx];
...
void calculate_permutation_matrices_for_all_3N_channels(double** P123_sparse_val_array,
	...
	// [EN] The paper emphasizes that P is expensive but energy-independent for a fixed packet basis. This routine pays
	// that one-time construction cost, stores the result, and then lets all later Faddeev solves reuse the same sparse
	// operator. / [CN] 论文强调：在固定波包基下，P 的构造代价很高，但它本身与能量无关。这个例程负责支付这次性成本、
	// 存盘，并让后续所有 Faddeev 求解复用同一个稀疏算符。
