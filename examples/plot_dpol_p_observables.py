...
"""
Purpose:
  Render PNG figures for multi-energy dpol-p observables generated by
...
Calls / dependencies:
  - Reads artifacts produced by `run_dpol_p_observables.py`.
  - Uses matplotlib in non-interactive `Agg` backend.

Usage:
  python3 examples/plot_dpol_p_observables.py \
...
def main() -> int:
    ...
    # Combined overview figure to compare all energies on the same axes.
