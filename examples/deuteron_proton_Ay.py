...
"""
Purpose:
  Run Tic-tac solver for a single target lab energy (default 190 MeV/u).
...
def write_solver_inputs(cfg: SolverRunConfig) -> tuple[Path, Path, Path, Path]:
    ...
    energy_file = cpp_input_dir / "energy_190_validation_autogen.txt"
    energy_file.write_text(f"{cfg.target_tlab:.6f}\n", encoding="utf-8")

    config_file = cpp_input_dir / "input_190_solver_validation_autogen.txt"
    config_file.write_text(
        "\n".join(
            [
                "# Autogenerated 190MeV solver validation config",
