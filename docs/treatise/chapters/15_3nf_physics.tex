...
\textbf{本节锁定 Tic-tac 中的弹性 AGS 算符顺序约定}。Deltuva
PRC 80 064002 (2009) eq.~(7a) 从全预解算符推导出相同粒子的对称化
弹性 AGS 方程。利用 \(tG_0=VG\) 与
\(G_0(1+tG_0)=G\)，其迭代核无须交换任何非对易因子即可写作
\begin{equation}
\label{eq:ch15-AGS-kernel}
K_{\rm AGS} = \Pop\V + (1 + \Pop)\,W^{(1)},
\qquad
\Pop \equiv \Pop_{123} + \Pop_{132},
\end{equation}
即 \((1+\Pop)\) 在\textbf{左}、\(W^{(1)}\) 在\textbf{右}。连续
Faddeev breakup 分量方程中的 \(W^{(1)}(1+\Pop)\) 属于另一个未知算符；
不能把该次序直接搬到弹性 channel-resolvent 方程。早期版本把 Witała
PRC 77 034004 (2008) eq.~(3) 误引为 3NF 方程；该式实际是基底耦合关系。

\texttt{calculate\_CPVC\_col} 按
\(\Pop\V\C + W^{(1)}\C + \Pop W^{(1)}\C\) 组装核，正是
\cref{eq:ch15-AGS-kernel} 的实现。

稠密算符顺序校验在 \texttt{tests/cpp/test\_faddeev\_operator\_order.cpp}：
使用满足 \(\Pop^2=\Pop+2\) 的置换和 Hermitian、非对易 toy
\(W^{(1)},V\)，逐项验证 Deltuva eq.~(7a) 化简为
\([\Pop V+(1+\Pop)W^{(1)}]G\)，并拒绝
\([\Pop V+W^{(1)}(1+\Pop)]G\)。
