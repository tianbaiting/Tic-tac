...
| Factorized W1 performance | `output/validation/n2lo_3nf_factorized_performance.json` | **Verified implementation result:** a fixed single-thread 200-channel-pair pass fell from `233.55 s` at `208159a` to `65.50 s` at `b39901e` (3.57x).  Subsequent magnetic factorization reduced the production `Np=4,Nq=3`, 101-block radial-order-one W1 build from `102.3 s` to `31.0 s`; channel batching reduced the radial-order-two build from `162.4 s` to `87.8 s`.  Exact-Hermitian reuse at `d14d2bb` directly integrates 56 blocks, transpose-fills 45, and reduces the same three-thread build to `48.4 s` (another 1.81x).  Commit `2563e8c` prevents the P123-specific three-thread limit from leaking into W1.  Commit `9eaa5f8` then fuses the shared `c1/c3` reduced integration and removes repeated momentum powers, ordered-cache lookup, and weight-table mutex traffic: fresh 1/16/96-thread builds fall from `126.5/16.9/12.7 s` to `58.0/12.3/11.1 s` (2.18x/1.37x/1.14x).  U and convergence outputs are byte-identical before/after and across all three thread counts. |
...
the projection hot-path optimization reduces the same build to 11.1 seconds.
Linear cell-count scaling still gives about 21.2 hours for a single low-order
